...
func formatSample(order []string, s Sample) string {
	parts := make([]string, 0, len(order)+2)
	for _, k := range order {
		v := s.Values[k]
		parts = append(parts, fmt.Sprintf("%s=%s", k, fmt4(v)))
	}
	parts = append(parts, fmt.Sprintf("y=%s", fmt4(s.Y)))
	if s.OK {
		parts = append(parts, "OK")
	} else {
		parts = append(parts, "NG")
	}
...
func main() {
	...
	maxIters := int64(10_000_000) // 繰り返し回数

	maxOKSave := 10 // 保存する正解の数
	maxNGSave := 10 // 保存する不正解の数

	// 進行状況を更新するタイミング。200_000回ごとに更新など
	printEvery := int64(200_000)

	// seedを自分で決めてもよい。最初はプログラムを実行した時刻をもとに乱数を発生
	seed := time.Now().UnixNano()

	// xlsx 出力（空文字なら保存しない）
	xlsxFile := "result.xlsx" // "" にすると保存はせず表示だけ
